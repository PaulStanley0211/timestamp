...
export function buildVideoFilter(look, cfg, { inLabel = '0:v', outLabel = 'vout', burnIn = [] } = {}) {
  ...
  // THERE IS NO TIMING NUDGE HERE ANY MORE (2026-09-05). A `setpts` used to sit
  // in front of this `fps`, scattering the frames the retiming duplicated so
  // they stopped arriving on a metronome. It was there because Seedance
  // delivered 24fps into a 25fps contract; Wan delivers 30fps, so this
  // decimates -- 450 frames to 375 -- and duplicates nothing. Measured on the
  // real segments before it was removed, with a Seedance tape as the control.
  // A source below 25fps brings the duplicates back and nothing warns about it:
  // see the note in config/look/base.json and the one test kept in
  // test/tapedeck-judder.test.js.
  const source = [
    `fps=fps=${fps}`,
