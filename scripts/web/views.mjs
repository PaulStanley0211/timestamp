...
export function faqItems({
  ...
  return [
    ...
    // "nobody else" is the promise (test/web-legal.test.js) and it stays; the
    // three-word sentence it now sits in is what breaks the disclosure's rhythm.
    { q: 'What happens to my photograph?', a: `It goes to fal.ai, the AI service that makes the video${classifier}. That's it, nobody else. Camera and location data are stripped the moment it arrives. It's deleted after ${photoDays} days and the tape after ${jobDays}, and you can delete either sooner from your account page.` },
    { q: 'How long does a tape take?', a: `A few minutes. You can watch the three phases on the status page, and if you close the tab the tape's still there when you come back.` },
    { q: 'Does it look real?', a: `Only the scene comes from the AI. Everything that makes it look like tape (the grain, the colour smear, the tear along the bottom of the frame, the date stamp) I built in ffmpeg, the way a 2003 camcorder put it there. Every file says it's AI-generated in its metadata.` },
    { q: 'Which shapes and qualities?', a: `${list(shapes)} at ${list(qualities)}. You get the file in the shape you picked, edge to edge in the wide ones, and ${sameInEveryShape ? 'a tape costs the same in every shape' : `the shape is part of the price, which you'll see on the order form before you press Record`}.` },
    { q: 'Can I delete everything?', a: 'Yes. One button on your account page deletes the account, your photo, your tapes and the credit history, all at once. Export first if you want a copy.' },
  ];
}
...
export function factCards({ frames = 375, fps = 25, shapes = [], photoDays = RETENTION_DEFAULTS.photoDays, jobDays = RETENTION_DEFAULTS.jobDays } = {}) {
  ...
  const cards = [
    ...
    ['fact--white', 'The shape you choose.', `${shapes.join(', ')}. Whichever you pick, that's the shape of the file you get, edge to edge.`],
    ['fact--white', `Deleted after ${photoDays} days.`, `That's your photo. The tape goes after ${jobDays} days, and you can delete either sooner yourself.`],
  ];
...
<header class="lime hero">
  ...
  <div class="hero-body">
    ...
    ${/* WRITTEN THE WAY THE OWNER WRITES, NOT THE WAY A MODEL DOES (2026-09-12).
         Matt Muir of Web Curios read the site cold and said the copy "reeks of
         machine": every sentence a comma-triad, not one contraction on the
         page, and the product defined by what it is not. The subline keeps
         his own pitch's shape (upload, pick, get back) and loses the second
         "drawer" on a screen whose manifesto already says it. "Your face"
         stays on purpose: asking for a waist-up photograph is the body-shape
         decision parked on branch body-shape-copy, not a copy edit. */''}
    <p class="hero-sub">Upload one photo of your face, pick a place and an outfit, and you get back a tape that looks like somebody filmed you on a camcorder.</p>
...
<section class="how2 inner">
  <article class="card how-card">
    ...
    <p class="card-d">The AI only makes the scene. The tape look I built separately in ffmpeg, because asking an AI for "VHS" just gives you clean footage with a warm filter. The grain goes on before the upscale so it clumps the way tape grain did, and the date stamp wears out with the picture. Drag the line.</p>
  </article>
  <article class="card how-card">
    ...
    <p class="card-d">${places.length ? `${capital(inWords(places.length))} places are on the menu. Or upload` : 'Upload'} a photo of your own (your garden, the kitchen you grew up in) and the tape is set there${places.length ? ' instead' : ''}.</p>
  </article>
</section>
...
<main>
  ...
  <p class="sub">Your photograph goes to fal.ai, the AI service that makes the
